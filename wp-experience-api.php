...
class WP_Experience_API {
	...
	public static function init() {
		...
		if ( ( is_multisite() && is_plugin_active_for_network( 'wp-experience-api/wp-experience-api.php' ) ) || defined( 'WP_XAPI_MU_MODE' ) ) {
			if ( ! empty( WP_Experience_API::$site_options ) || ! empty( WP_Experience_API::$site_options['wpxapi_network_lrs_password'] ) && ! empty( WP_Experience_API::$site_options['wpxapi_network_lrs_username'] ) && ! empty( WP_Experience_API::$site_options['wpxapi_network_lrs_url'] ) ) {
				...
				error_log( 'Please tell Network Administrator to set the default username/password/URL of the LRS (for plugin: WP Experience API)' );
			}
		} else {
			if ( ! is_multisite() &&
				...
				error_log( 'Please tell the Site Administrator to set the username/password/URL of the LRS (for plugin: WP Experience API)' );
			}
		}
	...
	public static function send( $trigger, $args ) {
		...
		/* //sanity check as actor/verb/object is required.  the rest is recommended.
		if ( empty( $actor ) || empty( $verb ) || empty( $object ) ) {
			return false;
		} */

		if ( empty( $verb ) || empty( $object ) ) {
			return false;
		}

		if ( empty( $actor ) ) {
			$statement['actor'] = 'Guest';
		}
...
/**===========================================================**/
	...
	public static function config_unset_notice() {
		?>
		<div class="error">
			<p><?php esc_html_e( 'Please tell Network Administrator to set the default username/password/URL of the LRS (for plugin: WP Experience API)', 'wpxapi' ); ?></p>
		</div>
		<?php
	}
	...
	*/
	public static function config_unset_local_notice() {
		?>
		<div class="error">
			<p><?php esc_html_e( 'Please tell the Site Administrator to set the username/password/URL of the LRS (for plugin: WP Experience API)' ); ?></p>
		</div>
		<?php
	}
	...
	public static function wpxapi_run_the_queue() {
		if ( WP_Experience_API::wpxapi_queue_is_not_empty() ) {
			...
			while ( $i < $count ) {
				...
				if ( false === (bool) $response->success ) {
					//failed, so enqueue!
					// WP_Experience_API::wpxapi_queue_enqueue( $queue_obj, $lrs );
					WP_Experience_API::wpxapi_queue_enqueue( $queue_obj, $lrs );
				}
			}
		}
